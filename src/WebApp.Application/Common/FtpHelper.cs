...
namespace WebApp.Common
{
    public class FtpWeb
    {
        ...
        public void Upload(string filename, byte[] byteFileBuff)
        {
            ...
            reqFTP.UseBinary = true;
            reqFTP.Proxy = null;
            reqFTP.ContentLength = byteFileBuff.Length;         
            Stream requestStream = reqFTP.GetRequestStream();        
            requestStream.Write(byteFileBuff, 0, byteFileBuff.Length);
            requestStream.Close();
        ...
        public byte[] GetFileByte(string fileName)
        {
            ...
            wc.Credentials = new NetworkCredential(ftpUserID, ftpPassword);
            wc.Proxy = null;
            byte[] FileData = wc.DownloadData(ftpURI + fileName);
            return FileData;
        }
        #endregion
    ...
    public class Insert_Standard_ErrorLog
    {
        ...
        {
            //LogHelper.Error(y);
            throw new UserFriendlyException("操作失败", y);
        }
    }
}
